...
Promise.all([
  ...
  // State: current index per card
  var state = {};
  if(content.frameCard) state[content.frameCard.id] = 0;
  ...
  // Helper function to wire up a card
  function renderCard(fight, points){
    var card = document.createElement('div');
    card.setAttribute('data-fight', fight.id);
    var isFrame = fight.id === 'frame';
    ...
    // Build dots
    var dotColor = isFrame ? '#ffffff' : fight.color;
  ...
  // Render frameCard first if it exists
  if(content.frameCard){
    var framePoints = tpDB[content.frameCard.id] || ['No talking points loaded.'];
    state[content.frameCard.id] = 0;
    renderCard(content.frameCard, framePoints);
  }

  // Render normal fights
  content.fights.forEach(function(fight){
    var points = tpDB[fight.id] || ['No talking points loaded.'];
    state[fight.id] = 0;
    renderCard(fight, points);
  });
